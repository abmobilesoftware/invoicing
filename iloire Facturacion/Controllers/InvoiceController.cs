   ...
   public class InvoiceController : BaseController
    {
        ...
        public JsonResult CreateInvoices()
        {
        ...
        public JsonResult ResetSubscriptions()
        {
           AutoResetSubscriptions();
           return null;
        }

        private bool AutoResetSubscriptions()
        {
           ...
           foreach (Company company in billedToday)
           {
              ...
              if (lastInvoice == null)
              {
                 canProceed = GenerateAutoInvoiceForCompany(company);                 
              }  
              else {
        ...
        private bool CreateAutoInvoice()
        {
           int today = DateTime.Now.Day;
           var billedToday = (from cp in db.Companies where cp.SubscriptionDetail.BillingDay == today select cp).ToList();
           foreach (Company company in billedToday)
           {
              bool opResult = GenerateAutoInvoiceForCompany(company);
              if (!opResult) return false;
           }
           return true;
        }

        private bool GenerateAutoInvoiceForCompany(Company company)
        {
